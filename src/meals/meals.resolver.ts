...
export class MealsResolver {
  ...
  findAllForRestaurant(@Args('limitEntity') limitEntity: LimitEntity) {
    if(!isValidObjectId(limitEntity?.user)) throw new BadRequestException("There isn't restaruant with this id");
  ...
  stateMeal(@Args('stateInput') stateInput: StateInput) {
    if(!isValidObjectId(stateInput?.id)) throw new BadRequestException("There isn't meal with this id");
