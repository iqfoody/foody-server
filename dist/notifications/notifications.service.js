...
let NotificationsService = class NotificationsService {
    ...
    async sendPrivate(createNotificationInput) {
        const notification = new this.NotificationsModel(createNotificationInput);
        if (createNotificationInput?.user) {
            const user = await this.usersService.findDeviceToken(createNotificationInput.user);
            if (user?.deviceToken)
                await this.firebaseService.sendPrivate(notification, user.deviceToken);
        }
            ...
            if (driver?.deviceToken)
                await this.firebaseService.sendPrivate(notification, driver.deviceToken);
        }
        return;
    }
