...
let TransactionsService = class TransactionsService {
    ...
    async updateTransaction(updateTransactionInput) {
        ...
        if (!transaction)
            throw new common_1.BadRequestException("There isn't transaction with this order");
    ...
    async findAllAdmin(limitEntity) {
        const skipIndex = limitEntity.page * limitEntity.page;
        const transactions = await this.TransactionsModel.find({ admin: limitEntity?.user }).populate([{ path: "user", select: { name: 1, image: 1 } }, { path: "driver", select: { name: 1, image: 1 } }]).sort({ _id: -1 }).limit(limitEntity.limit).skip(skipIndex);
